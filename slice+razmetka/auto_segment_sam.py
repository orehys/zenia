...
def auto_segment_forest(images_dir, output_dir, checkpoint_path="sam_vit_h_4b8939.pth"):
    """
    ...
    device = "cpu"  # ← Принудительно процессор, остальное не трогаем!
    print(f"🖥️  Устройство: {device}")

    sam = sam_model_registry["vit_h"](checkpoint=checkpoint_path)
    sam.to(device=device)  # ← Загружаем модель в оперативную память
    sam.eval()
    sam.eval()
